...
!-----------------------------------------------------------------------
  ...
  implicit none
  integer,intent(in):: inode,ipar,nsd,nrc,tag,mpi_world
  real(rp),intent(in):: bufs(nsd)
  real(rp),intent(out):: bufr(nrc)
  integer:: status(MPI_STATUS_SIZE),ierr,i

!-----Even: send & recv
  if (ipar.eq.0) then
    call mpi_send(bufs,nsd,mpi_real_rp,inode,tag,mpi_world,ierr) 
    call mpi_recv(bufr,nrc,mpi_real_rp,MPI_ANY_SOURCE,tag, &
