...
def write(nsys,fname="pmdini",format=None):
...
def write_dump(nsys,fname='dump'):
    """
    ...
    f.write("ITEM: ATOMS id type x y z vx vy vz")
    aux_names = nsys.get_aux_names()
    if len(aux_names)>0:
        for name in aux_names:
            f.write(" {0:s}".format(name))
    # if (nsys.aux_names and nsys.auxs) and \
    #    len(nsys.aux_names) == len(nsys.auxs.T):
    #     for iaux,aux in enumerate(nsys.aux_names):
    ...
    if len(aux_names)>0:
        for i in range(len(nsys.atoms)):
            ...
            #...TODO: nsys.auxs and nsys.aux_names are obsolete use nsys.atoms
            # aux = nsys.auxs[i]
            # for iaux in range(len(nsys.aux_names)):
...
def write_cube(nsys, fname='cube'):
    """
