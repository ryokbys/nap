#!/usr/bin/env python
"""
Create in.params.uf3 file.

Usage:
  {0:s} [options] CONFIG_UF3_FILE
...
Options:
  ...
  --init3b INIT3B  Initial value of 3-body potential coefficients. [default: 0.0]
"""
import os, sys
from docopt import docopt
import numpy as np
from nappy.fitpot.uf3util import write_params_uf3
...
def create_params(config,
    ...
    for t in trios:
        ...
        d3b['rcjk'] = t['cutoffs'][0] + t['cutoffs'][1]
        d3b['ncij'] = resolution3 +3
        d3b['ncik'] = resolution3 +3
        d3b['ncjk'] = resolution3*2 +3
        d3b['nkij'] = d3b['ncij'] +4
        d3b['nkik'] = d3b['ncik'] +4
        d3b['nkjk'] = d3b['ncjk'] +4
        dkij = (d3b['rcij']-rmin3)/(d3b['nkij']-8+1)
        d3b['knotsij'] = [ rmin3 for i in range(d3b['nkij']) ]
        d3b['knotsij'][-4:] = [ d3b['rcij'] for i in range(4) ]
        d3b['knotsij'][3:-4] = [ rmin3 +dkij*i for i in range(resolution3) ]
        dkik = (d3b['rcik']-rmin3)/(d3b['nkik']-8+1)
        d3b['knotsik'] = [ rmin3 for i in range(d3b['nkik']) ]
        d3b['knotsik'][-4:] = [ d3b['rcik'] for i in range(4) ]
        d3b['knotsik'][3:-4] = [ rmin3 +dkik*i for i in range(resolution3) ]
        dkjk = (d3b['rcjk']-rmin3)/(d3b['nkjk']-8+1)
        d3b['knotsjk'] = [ rmin3 for i in range(d3b['nkjk']) ]
        d3b['knotsjk'][-4:] = [ d3b['rcjk'] for i in range(4) ]
        d3b['knotsjk'][3:-4] = [ rmin3 +dkjk*i for i in range(resolution3*2) ]
        d3b['coefs'] = np.zeros((d3b['ncij'],d3b['ncik'],d3b['ncjk']))
        prms['3B'].append(d3b)
    ...
    if ans != 'y':
        ...
        print(f' --> {outfname}')
        write_params_uf3(prms, outfname=outfname,
...
def prms_to_fp(prms,
    ...
    for d3b in prms['3B']:
        ...
        ncik = d3b['ncik']
        ncjk = d3b['ncjk']
        rc3max = max(rc3max, d3b['rcij'], d3b['rcik'])
        coefs = d3b['coefs']
        nlead = d3b['nlead']
        ntrail= d3b['ntrail']
        for i in range(ncij):
